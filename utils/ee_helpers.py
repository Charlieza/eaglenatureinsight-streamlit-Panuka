...
def annual_soil_moisture_collection(geom: ee.Geometry, start_year: int, end_year: int) -> ee.FeatureCollection:
    ds = get_datasets()
    years = ee.List.sequence(max(2015, start_year), end_year)
...
def annual_evapotranspiration_collection(geom: ee.Geometry, start_year: int, end_year: int) -> ee.FeatureCollection:
    ds = get_datasets()
    years = ee.List.sequence(max(2000, start_year), end_year)
...
def rainfall_variability_pct(geom: ee.Geometry, start_year: int, end_year: int):
    fc = annual_rain_collection(geom, start_year, end_year).filter(ee.Filter.notNull(["value"]))
    mean = ee.Number(fc.aggregate_mean("value"))
    std = ee.Number(fc.reduceColumns(ee.Reducer.stdDev(), ["value"]).get("stdDev"))
    return ee.Algorithms.If(mean.gt(0), std.divide(mean).multiply(100), None)


def rainfall_reliability_index(geom: ee.Geometry, start_year: int, end_year: int):
    var_pct = rainfall_variability_pct(geom, start_year, end_year)
    return ee.Algorithms.If(
        ee.Algorithms.IsEqual(var_pct, None),
        None,
        ee.Number(100).subtract(ee.Number(var_pct)).max(0).min(100),
    )


def rainfall_trend_pct(geom: ee.Geometry, start_year: int, end_year: int):
    fc = annual_rain_collection(geom, start_year, end_year).filter(ee.Filter.notNull(["value"])).sort("year")
    count = ee.Number(fc.size())
    return ee.Algorithms.If(
        count.gte(6),
        ee.Number(ee.FeatureCollection(fc.toList(3, count.subtract(3))).aggregate_mean("value"))
        .subtract(ee.Number(ee.FeatureCollection(fc.toList(3, 0)).aggregate_mean("value")))
        .divide(ee.Number(ee.FeatureCollection(fc.toList(3, 0)).aggregate_mean("value")))
        .multiply(100),
        None,
    )


def soil_moisture_anomaly_mean(geom: ee.Geometry, hist_end: int):
    ds = get_datasets()
    current = soil_moisture_mean(geom, hist_end)
    baseline_coll = (
        ds["SMAP_L4"]
        .filterBounds(geom)
        .filterDate("2015-01-01", f"{hist_end}-12-31")
        .select("sm_surface")
    )
    baseline = baseline_coll.mean().reduceRegion(
        reducer=ee.Reducer.mean(), geometry=geom, scale=11000, maxPixels=1e13, bestEffort=True
    ).get("sm_surface")
    return ee.Algorithms.If(
        ee.List([ee.Algorithms.IsEqual(current, None), ee.Algorithms.IsEqual(baseline, None)]).contains(True),
        None,
        ee.Number(current).subtract(ee.Number(baseline)),
    )


def distance_to_water_m(geom: ee.Geometry):
    ds = get_datasets()
    water_mask = ds["GSW"].gte(20)
    distance = water_mask.Not().fastDistanceTransform(256).sqrt().multiply(30).rename("distance_m")
...
def slope_mean(geom: ee.Geometry):
    ds = get_datasets()
    slope_img = ee.Terrain.slope(ds["SRTM"]).rename("slope")
    return slope_img.reduceRegion(
        reducer=ee.Reducer.mean(),
        geometry=geom,
        scale=30,
        maxPixels=1e13,
...
def wetness_index_mean(geom: ee.Geometry):
    ds = get_datasets()
    dem = ds["SRTM"]
    wetness = dem.focal_min(3).subtract(dem).rename("wetness")
    return wetness.reduceRegion(
        reducer=ee.Reducer.mean(),
        geometry=geom,
        scale=30,
        maxPixels=1e13,
...
def woody_cover_pct(geom: ee.Geometry):
    ds = get_datasets()
    total_area_raw = ee.Image.pixelArea().reduceRegion(
        reducer=ee.Reducer.sum(), geometry=geom, scale=10, maxPixels=1e13, bestEffort=True
    ).get("area")
    total_area = safe_number(total_area_raw, 0)

    woody_mask = ds["WORLDCOVER"].eq(10).Or(ds["WORLDCOVER"].eq(20))
    woody_area_raw = ee.Image.pixelArea().updateMask(woody_mask).reduceRegion(
        reducer=ee.Reducer.sum(), geometry=geom, scale=10, maxPixels=1e13, bestEffort=True
    ).get("area")
    woody_area = safe_number(woody_area_raw, 0)
    return ee.Algorithms.If(total_area.gt(0), woody_area.divide(total_area).multiply(100), None)


def water_proximity_with_polygon(geom: ee.Geometry) -> ee.Image:
    ds = get_datasets()
    water_mask = ds["GSW"].gte(20)
    distance = water_mask.Not().fastDistanceTransform(256).sqrt().multiply(30).rename("distance_m")
    vis = distance.visualize(
        min=0,
        max=3000,
...
def terrain_wetness_with_polygon(geom: ee.Geometry) -> ee.Image:
    ds = get_datasets()
    dem = ds["SRTM"]
    wetness = dem.focal_min(3).subtract(dem).rename("wetness")
    vis = wetness.visualize(
        min=-5,
        max=5,
...
def compute_metrics(geom: ee.Geometry, hist_start: int, hist_end: int, last_full_year: int):
    ...
    metric_exprs = {
        ...
        "rain_variability_pct": rainfall_variability_pct(geom, hist_start, hist_end),
        "rain_reliability_index": rainfall_reliability_index(geom, hist_start, hist_end),
        "rain_trend_pct": rainfall_trend_pct(geom, hist_start, hist_end),
        "lst_mean": lst_recent_mean_from_range(geom, hist_start, hist_end),
        "tree_pct": landcover_pct(geom, 10),
        "woody_cover_pct": woody_cover_pct(geom),
        "cropland_pct": landcover_pct(geom, 40),
        "built_pct": landcover_pct(geom, 50),
        "water_occ": surface_water_occurrence_mean(geom),
        "distance_to_water_m": distance_to_water_m(geom),
        "slope_mean": slope_mean(geom),
        "wetness_index": wetness_index_mean(geom),
        ...
        "soil_moisture_anomaly": soil_moisture_anomaly_mean(geom, hist_end),
