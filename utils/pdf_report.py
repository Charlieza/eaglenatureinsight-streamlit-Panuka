...
def build_pdf_report(
    ...
    **kwargs,
    ...
    for txt in [
        ...
        "Where one dataset under-reads sparse tree cover or seasonal water, this report now cross-checks those themes with additional forest and water layers so that false zero values are less likely.",
    ...
    story.append(_metric_table([
        ...
        ("Dynamic World tree signal", fmt_num(metrics.get("dynamic_world_tree_prob_pct"), 1, "%")),
        ...
        ("Near-site water context", fmt_num(metrics.get("context_water_occ"), 1)),
        ("Maximum water extent", fmt_num(metrics.get("water_max_extent_pct"), 1, "%")),
        ("Seasonal water area", fmt_num(metrics.get("seasonal_water_area_pct"), 1, "%")),
    ...
    indicator_rows = [
        ...
        [
            ...
            fmt_num(metrics.get("water_max_extent_pct"), 1, "%"),
            f"Maximum water extent is {fmt_num(metrics.get('water_max_extent_pct'), 1, '%')}, which gives a fuller view of where water has been detected over time even if current visible water is limited.",
            f"Use the maximum water-extent value of {fmt_num(metrics.get('water_max_extent_pct'), 1, '%')} to review drainage lines, buffers, and water-planning decisions.",
        ],
        [
            "Tree signal",
            fmt_num(metrics.get("dynamic_world_tree_prob_pct"), 1, "%"),
            f"Dynamic World tree probability is {fmt_num(metrics.get('dynamic_world_tree_prob_pct'), 1, '%')}, which acts as a supplementary woody-vegetation signal where a strict land-cover tree class may under-read sparse tree cover.",
            f"Use the tree-signal value of {fmt_num(metrics.get('dynamic_world_tree_prob_pct'), 1, '%')} together with the categorical tree-cover result when interpreting sparse or fragmented tree cover.",
        ],
        [
            "Soil moisture",
    ...
    story.append(_metric_table([
        ...
        ("Tree cover", fmt_num(metrics.get("tree_pct"), 1, "%")),
        ("WorldCover tree class", fmt_num(metrics.get("worldcover_tree_pct"), 1, "%")),
        ("Hansen current tree cover", fmt_num(metrics.get("hansen_tree_pct"), 1, "%")),
        ("Dynamic World tree signal", fmt_num(metrics.get("dynamic_world_tree_prob_pct"), 1, "%")),
        ("Cropland", fmt_num(metrics.get("cropland_pct"), 1, "%")),
        ...
        ("Near-site water context", fmt_num(metrics.get("context_water_occ"), 1)),
        ("Maximum water extent", fmt_num(metrics.get("water_max_extent_pct"), 1, "%")),
        ("Permanent water area", fmt_num(metrics.get("permanent_water_area_pct"), 1, "%")),
        ("Seasonal water area", fmt_num(metrics.get("seasonal_water_area_pct"), 1, "%")),
    ], (7.2 * cm, 9.8 * cm)))
    _section_rule(story)
    ...
    story.append(_metric_table([
        ...
        ("Maximum water extent", f"{fmt_num(metrics.get('water_max_extent_pct'), 1, '%')} — Wider long-term water footprint used to reduce false dry readings."),
        ("Permanent water area", f"{fmt_num(metrics.get('permanent_water_area_pct'), 1, '%')} — Area with year-round water signal."),
        ("Seasonal water area", f"{fmt_num(metrics.get('seasonal_water_area_pct'), 1, '%')} — Area with seasonal water signal."),
        ("Average water seasonality", f"{fmt_num(metrics.get('water_seasonality_months'), 1)} — Average number of months with water where water is present."),
        ("Recent monthly water presence", f"{fmt_num(metrics.get('water_presence_frequency'), 1, '%')} — Recent monthly water-detection frequency from JRC monthly history."),
        ("Dynamic World water signal", f"{fmt_num(metrics.get('dynamic_world_water_prob_pct'), 1, '%')} — Supplementary near-real-time optical water probability."),
        ("Current NDWI", f"{fmt_num(metrics.get('ndwi_current'), 3)} — Recent open-water / wetness signal from Sentinel-2."),
        ("Current MNDWI", f"{fmt_num(metrics.get('mndwi_current'), 3)} — Recent water-sensitive index that can help for smaller sites."),
        ("Current NDMI", f"{fmt_num(metrics.get('ndmi_current'), 3)} — Recent moisture signal from Sentinel-2."),
